...
def _before_model_callback(
    ...
    """ADK before-model callback — log token count, enforce input safety."""
    agent_name = callback_context.agent_name
    msg_count = len(llm_request.contents or [])
    logger.info("[ADK] before_model agent=%s messages=%d", agent_name, msg_count)
...
def _after_model_callback(
    ...
    """ADK after-model callback — log response, redact PII if needed."""
    agent_name = callback_context.agent_name
    parts = llm_response.content.parts if llm_response.content else []
    text = parts[0].text[:80] if parts else ""
    logger.info("[ADK] after_model agent=%s response_preview=%r", agent_name, text)
    return None  # None = use original response


# Wire callbacks into root agent
...
app = FastAPI(
    ...
    description="AI lettings agent — ADK + LangGraph + A2A + JWT",
    version="0.1.0",
)
...
class TokenRequest(BaseModel):
    client_id: str
    client_secret: str  # POC: any non-empty value issues a token
...
async def health():
    return {
        ...
        "features": ["adk", "langgraph", "a2a", "jwt"],
    }


@app.get("/.well-known/agent.json")
async def agent_card_endpoint():
    """A2A AgentCard — describes this agent to external orchestrators."""
    return JSONResponse(content=agent_card)


@app.post("/auth/token")
async def issue_token(req: TokenRequest):
    """Issue a JWT. POC: any non-empty client_secret is accepted."""
...
async def chat(req: ChatRequest, claims: dict = Depends(verify_token)):
    ...
    Runs root_agent with all sub-agents via ADK Runner.
    Before/after model callbacks log each LLM call.
    """
...
async def workflow_endpoint(req: WorkflowRequest, claims: dict = Depends(verify_token)):
    ...
    Runs the full lettings StateGraph: qualify → match? → book → followup.
    Returns the last AI message and current pipeline stage.
    """
